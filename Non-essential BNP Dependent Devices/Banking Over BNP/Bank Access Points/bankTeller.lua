...
local configFile = "bankTeller.conf"
...
local function buildATM(accountInfo)
	...
	local function deposit()
		...
		button:onClick(function()
			local packet = { type="BALANCE_UPDATE", deposit = 0, user = username, pass = accountInfo.pass, bankTeller = true }
			if submitted1 then
				if total > 0 then
					popup:destroy()
	...
	local function withdrawl()
		local function specificWithdrawl()
			...
			popup:addButton():setSize(4,1):setPosition(26,6):setText("Send"):onClick(function()
				if next(coins) then
					sendPacket({type="WITHDRAWL",request=coins,user=username,pass=accountInfo.pass,address=os.getComputerID()})
